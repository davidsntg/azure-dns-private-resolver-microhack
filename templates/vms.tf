...
resource "azurerm_storage_account" "spoke02-sa" {
  name                     = "spoke02sa${random_string.asa.result}"
...
resource "azurerm_linux_virtual_machine" "spoke02-vm" {
  ...
  boot_diagnostics {
    storage_account_uri = azurerm_storage_account.spoke02-sa.primary_blob_endpoint
  }
